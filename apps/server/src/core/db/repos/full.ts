...
import { camelCase } from "@/utils/string/camel-case";
...
> {
   ...
   */
  // biome-ignore lint/correctness/noUnusedPrivateClassMembers: Used in subclasses
    ...
    if (err instanceof Error && "code" in err) {
      switch (err.code) {
        case "23505": {
          ...
          const keyMatch = detail?.match(keyRegex);
          const valueMatch = detail?.match(valueRegex);

          if (keyMatch && valueMatch) {
            const keys = keyMatch[1].split(", ").map((key) => key.trim());
            const _values = valueMatch[1].split(", ").map((value) => value.trim());
            const fieldErrors: Record<string, string[]> = {};
            const _isComposite = keys.length > 1;
   ...
   *
   * @param {PgInsertValue<U>} _row
   * @returns {Promise<void>}
   */
  async beforeCreate(_row: PgInsertValue<U>) {}

  /**
   * A hook that is invoked after a row is inserted and right before returning to the caller.
   *
   * @param {InferSelectModel<U>} _row
   * @returns {Promise<InferSelectModel<U>>}
   */
  async afterCreate(_row: InferSelectModel<U>) {}
   ...
   * @returns {Promise<void>}
   */
  async afterDelete(_row: InferSelectModel<U>) {}
   ...
   * @returns {Promise<void>}
   */
  async afterFind(_row: InferSelectModel<U>) {}

  /**
   * A hook that is invoked right before a row is updated.
   *
   * @param {PgUpdateSetSource<U>} _row
   * @returns {Promise<void>}
   */
  async beforeUpdate(_row: PgUpdateSetSource<U>) {}
   ...
   * @returns {Promise<void>}
   */
  async afterUpdate(_row: InferSelectModel<U>) {}
  ...
  async create<TSelectedFields extends SelectedFieldsFlat>(
    ...
    opts: {
      ...
      onConflictDoUpdate?: PgInsertOnConflictDoUpdateConfig<
        PgInsertBase<U, NodePgQueryResultHKT, undefined, Record<string, unknown>, never>
      >;
      tx?: Transaction<T>;
    },
  ...
  async createMany<TSelectedFields extends SelectedFieldsFlat>(
    ...
    opts: { columns: TSelectedFields; tx?: Transaction<T> },
  ...
  async createMany<TSelectedFields extends SelectedFieldsFlat>(
    values: PgInsertValue<U>[],
    opts?: { columns?: TSelectedFields; tx?: Transaction<T> },
  ): Promise<SelectResultFields<TSelectedFields>[] | InferSelectModel<U>[]> {
    try {
  ...
  >(opts?: {
    ...
    where?: QConfig["where"];
    tx?: Transaction<T>;
  }): Promise<SelectResultFields<TSelectedFields>[] | InferSelectModel<U>[] | null> {
  ...
  async findMany<QConfig extends FindManyQueryConfig<T, V>>(opts?: FindManyOpts<QConfig>) {
    ...
    const qb = tx || this.db.orm;
    // @ts-expect-error
    const rows = await qb.query[this.#modelName].findMany(config || {});
...
/**
 * Example usage:
 * export const appDb: AppDb<Schema> = { orm: db, url: DATABASE_URL, schema };
 *
 * class MakeRepository extends Repository<Schema, Schema["makes"], "makes"> {
 *   async beforeCreate() {}
 *   async afterCreate() {}
 *   async afterDelete() {}
 *   async afterFind() {}
 ...
 * }
 *
 * export const makeRepository = new MakeRepository(appDb, makes);
 *
 */
