...
const worker = new Worker<{ id: string }>(
  ...
  async (job) => {
    ...
    const promises = posts.valid.map((post) =>
      limit(async () => {
        const success = await downloadTwitter(post);
    ...
    await TwitterPostModel.updateMany(
...
export function checkTwitterWorkerStatus() {
  if (worker.isRunning()) {
