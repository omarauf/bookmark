...
import type { ListTiktokPosts } from "@workspace/contracts/tiktok/post";
...
export function Filter({ className }: { className?: string }) {
  ...
  const search = useSearch({ from: "/tiktok/" });
  const navigate = useNavigate({ from: "/tiktok" });

  const setFilter = useCallback(
    (newFilter: ListTiktokPosts) => navigate({ search: { ...search, ...newFilter } }),
    [search, navigate],
  );
