...
type Props = {
  media: PopulatedTwitterPost["media"];
  className?: string;
};
...
export function TwitterMediaCard({ media, className }: Props) {
  ...
  if (media.length === 1) {
    ...
    return (
      <img
        key={m.url}
        className={cn(
          "max-h-[420px] rounded-2xl border bg-black object-cover",
          m.width / m.height > 1 ? "w-full" : "h-full",
          m.mediaType === "image" && "mx-auto",
          className,
        )}
        src={staticFile(m.mediaType === "image" ? m.url : m.thumbnail)}
        alt="media-0"
      />
    );
  }
  ...
  if (media.length === 3) {
    return (
      <div
        className={cn(
          "grid max-h-[420px] grid-cols-2 grid-rows-2 gap-0.5 overflow-hidden rounded-2xl",
          className,
        )}
      >
        {media.map((m, i) => (
