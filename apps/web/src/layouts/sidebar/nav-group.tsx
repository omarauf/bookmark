...
import type { NavCollapsible, NavGroupItem, NavItem, NavLink } from "./types";
              ...
              <SidebarMenuSubItem key={subItem.title}>
                <SidebarMenuSubButton asChild isActive={checkIsActive(href, subItem)}>
                  <Link to={subItem.url} onClick={() => setOpenMobile(false)}>
                    {subItem.icon && <Iconify icon={subItem.icon} />}
          ...
          {item.items.map((sub) => (
            <DropdownMenuItem key={`${sub.title}-${sub.url}`} asChild>
              <Link to={sub.url} className={`${checkIsActive(href, sub) ? "bg-secondary" : ""}`}>
                {sub.icon && <Iconify icon={sub.icon} />}
...
function checkIsActive(href: string, item: NavItem, mainNav = false) {
  return (
    href.startsWith(String(item.url)) || // /endpint?search=param
