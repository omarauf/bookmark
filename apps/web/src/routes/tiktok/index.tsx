...
import { PostDialog } from "@/features/tiktok/components/dialog";
import { Filter } from "@/features/tiktok/components/filter";
import { PostCard } from "@/features/tiktok/components/post/card";
import { Main } from "@/layouts/containers/main";
import { cn } from "@/lib/utils";

export const Route = createFileRoute("/tiktok/")({
  component: Tiktok,
  validateSearch: ListTiktokPostSchema,
  loaderDeps: ({ search }) => search,
  loader: async ({ context: { orpc, queryClient }, deps }) => {
    await queryClient.prefetchInfiniteQuery(
      orpc.tiktokPosts.list.infiniteOptions({
        initialPageParam: 1,
